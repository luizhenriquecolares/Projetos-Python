...
def adicionar_insumo():
    cursor.execute(f'''INSERT INTO Estoque (Produto, Quantidade, DataValidade, Lote) VALUES ("{nome_insumo.get()}", {qtde_insumo.get()}, "{data_insumo.get()}", {lote_insumo.get()})''')
    cursor.commit()
...
def deletar_insumo():
    if len(nome_insumo.get()) < 2:
        ...
        # finalizar a função e vai voltar para a função deletar insumo
        return
